...
class InputAutocompleteComponentProxy extends InputAutocompleteComponent {
  ...
  autogenerated.InputElement __e9;
  ...
  void created_autogenerated() {
    ...
    __t.conditional(__e7, () => inputHasFocus && filteredChoices != null, (__t) {
      ...
      __e5 = __e6.nodes[1];
      __t.loop(__e5, () => filteredChoices, ($list, $index, __t) {
        var choice = $list[$index];
        var __e3, __e4;
        __e4 = __html2.clone(true);
        __e3 = __e4.nodes[1];
        var __binding2 = __t.contentBind(() =>  renderChoice(choice) , false);
        __e3.nodes.add(__binding2);
        __t.listen(__e4.onMouseDown, ($event) { mouseDownChoice(choice, $event); });
        __t.listen(__e4.onMouseOver, ($event) { mouseOverChoice(choice, $event); });
        __t.listen(__e4.onMouseUp, ($event) { selectChoice(choice); });
        __t.oneWayBind(() =>  choice.key , (e) { if (__e4.attributes['choice-key'] != e) __e4.attributes['choice-key'] = e; }, false, false);
        __t.bindClass(__e4, () =>  isFocused(choice) ? 'focused' : 'notfocused' , false);
    ...
    __t.bindClass(__e8, () =>  inputHasFocus && filteredChoices != null ? 'autocomplete-content-visible' : 'autocomplete-content-hidden' , false);
    __e9 = __root.nodes[1].nodes[3];
    __t.listen(__e9.onBlur, ($event) { inputBlur($event); });
    __t.listen(__e9.onFocus, ($event) { inputFocus($event); });
    __t.listen(__e9.onKeyDown, ($event) { $event = new autogenerated.KeyEvent($event); keyDown($event); });
    __t.listen(__e9.onKeyUp, ($event) { $event = new autogenerated.KeyEvent($event); keyUp($event); });
    __t.create();
  }
